...
export interface IProfileDataSource{
  create(userModel: UserModel): Promise<ICreateProfileResponse>;
}
...
export class RestProfileService implements IProfileDataSource {
  private path = '/api/v1/users/4/profile'
  ...
  async create(userModel: UserModel): Promise<ICreateProfileResponse> {
    try {
      const headers = {
      ...
      const {
        data 
      } = await this.axiosInstance.post('', userModel, { headers });
